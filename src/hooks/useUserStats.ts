...
export const useUserStats = (timeRange: TimeRange = 'medium_term') => {
	...
	const fetchData = useCallback(async () => {
		...
		try {
			...
			const [cachedTopArtists, cachedTopTracks, cachedRecentlyPlayed] =
				await Promise.all([
			...
			console.debug('API call results:', {
				...
				errors: {
					topArtists:
						topArtistsRes.status === 'rejected'
							? topArtistsRes.reason?.message
							: null,
					topTracks:
						topTracksRes.status === 'rejected'
							? topTracksRes.reason?.message
							: null,
					recentlyPlayed:
						recentlyPlayedRes.status === 'rejected'
							? recentlyPlayedRes.reason?.message
							: null,
				},
			});
			...
			if (
				...
				topArtistsRes.value.status !== 'cached'
			) {
				...
				Promise.race([
					...
					new Promise((_, reject) =>
						setTimeout(
							() => reject(new Error('Background cache timeout')),
							10000
						)
					),
				])
					.then(() => {
						console.debug(
							'✅ Top artists cached successfully for key:',
							cacheKey
						);
					})
					.catch((cacheError) => {
						...
						console.error(
							'❌ Data size:',
							JSON.stringify(mappedArtists).length,
							'chars'
						);
					});
			...
			console.debug(
				'✅ Top artists section completed, moving to top tracks...'
			);
			...
			if (
				...
				topTracksRes.value.status !== 'cached'
			) {
				try {
					...
					Promise.race([
						...
						.then(() => {
							console.debug(
								'✅ Top tracks cached successfully for key:',
								cacheKey
							);
						})
						.catch((cacheError) => {
							...
							console.error(
								'❌ Data size:',
								JSON.stringify(mappedTracks).length,
								'chars'
							);
						});
				} catch (error) {
			...
			console.debug(
				...
				recentlyPlayedRes.status
			);
			if (
				recentlyPlayedRes.status === 'fulfilled' &&
				recentlyPlayedRes.value.status !== 'cached'
			) {
				try {
					...
					Promise.race([
						...
						.then(() => {
							console.debug(
								'✅ Recently played cached successfully for key:',
								cacheKey
							);
						})
						.catch((cacheError) => {
							...
							console.error(
								'❌ Data size:',
								JSON.stringify(mappedRecentlyPlayed).length,
								'chars'
							);
						});
				} catch (error) {
