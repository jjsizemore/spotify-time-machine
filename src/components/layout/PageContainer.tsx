...
export default function PageContainer({
  ...
  if (isAuthLayout) {
    return (
      <div className="min-h-screen bg-spotify-black flex items-center justify-center">
        <main className={`p-6 ${maxWidthClass} mx-auto ${className}`}>{children}</main>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-spotify-black">
      <main className={`p-6 ${maxWidthClass} mx-auto ${className} pb-20`}>
        {title && <h1 className="text-3xl font-bold text-spotify-light-gray mb-2">{title}</h1>}
        {description && <p className="text-spotify-light-gray mb-8">{description}</p>}
        {children}
      </main>
    </div>
  );
