...
export default function TrackItem({
  ...
  const albumImage = track.album.images.find(img => img.height === 64) ||
  ...
  return (
    <div className="flex items-center bg-spotify-black/30 p-4 rounded-md hover:bg-spotify-medium-gray/30 transition">
      ...
      <div className="flex-grow min-w-0">
        <h3 className="text-spotify-white text-md font-medium truncate">{track.name}</h3>
        <p className="text-spotify-light-gray text-sm truncate">{artistNames}</p>
      ...
      <div className="hidden md:block text-right mr-4 flex-shrink-0 w-40">
        <p className="text-spotify-light-gray text-sm truncate">{track.album.name}</p>
      </div>
