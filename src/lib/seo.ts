...
import { PUBLIC_ENV } from './clientEnv';
...
export function generateEnhancedMetadata({
  ...
  const fullImageUrl = imageUrl.startsWith('http') ? imageUrl : `${SEO_CONFIG.baseUrl}${imageUrl}`;
...
export function generateWebApplicationSchema(overrides: Record<string, any> = {}) {
  return {
