...
export const debugEnabled = () => env.NEXT_PUBLIC_DEBUG === 'true';
  ...
  try {
    ...
    if (oldestKey) {
      try {
        ...
        console.info(
          `Cache cleanup: Removed oldest item (${oldestKey.replace(
            CACHE_PREFIX,
            ''
          )}) from ${new Date(oldestTimestamp).toLocaleString()}`
        );
        return true;
      } catch (removeError: any) {
        logCacheError({
    ...
    console.warn('Cache cleanup requested but no cache entries found to remove');
    return false;
  } catch (error: any) {
    logCacheError({
...
export function getCachedData<T>(key: string): T | null {
  ...
  try {
    ...
    if (age > item.ttl) {
      console.debug(
        `Cache expired: ${key} - age: ${Math.round(age / 1000)}s, TTL: ${Math.round(
          item.ttl / 1000
        )}s`
      );
      try {
    ...
    console.debug(
      `Cache hit: ${key} - age: ${Math.round(age / 1000)}s, remaining: ${Math.round(
        (item.ttl - age) / 1000
      )}s`
    );
    return item.data;
...
export async function setCachedDataCompressed<T>(
  ...
  if (useCompression) {
    ...
    if (compressedSize >= originalSize * 0.9) {
      ...
      compressed = false;
      console.debug(`Compression not beneficial for ${key}, using uncompressed`);
    }
  } else {
  ...
  if (isIndexedDBAvailable()) {
    try {
      ...
      if (indexedDBResult !== null) {
        ...
        console.debug(`❌ Not found in IndexedDB: ${key}, trying localStorage...`);
      }
    } catch (error) {
      console.warn('IndexedDB failed, falling back to localStorage:', error);
    }
...
export function clearAllCachesOnly(clearInMemoryCaches?: () => void): number {
  try {
    ...
    console.log(
      `Cache cleared: ${removedCount} localStorage items removed${
        clearInMemoryCaches ? ', in-memory caches cleared' : ''
      }`
    );
