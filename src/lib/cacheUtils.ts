...
function clearOldestCacheEntry(): boolean {
	...
	if (oldestKey) {
		...
		console.warn(
			`Cache full: Removed oldest item (${oldestKey}) to make space.`
		);
		return true;
	}
	return false;
...
export function setCachedData<T>(
	...
	try {
		...
		console.warn(
			'Initial error saving to localStorage:',
			error.name,
			error.message
		);
		...
		if (
			...
			console.warn(
				'LocalStorage quota exceeded. Attempting to clear oldest entry...'
			);
