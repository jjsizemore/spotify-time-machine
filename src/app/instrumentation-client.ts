...
function hasUserConsented(): boolean {
  ...
  try {
    ...
    if (consentData) {
      ...
      const hasConsent = Object.values(parsed).some((consent) => consent === true);
      if (process.env.NODE_ENV === 'development') {
        console.log('🍪 Consent check (c15t):', hasConsent ? 'granted' : 'not granted');
      }
      return hasConsent;
    }
    ...
    const hasAnalyticsGeneral = localStorage.getItem('analytics-consent') === 'true';
    const fallbackConsent = hasAnalyticsGa || hasAnalyticsGeneral;

    if (process.env.NODE_ENV === 'development' && fallbackConsent) {
      console.log('🍪 Consent check (fallback):', fallbackConsent ? 'granted' : 'not granted');
    }
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('🌍 PostHog: Geolocation API failed, assuming non-EEA user');
        }
        return false;
      }
    } catch (geoError) {
      if (process.env.NODE_ENV === 'development') {
        console.debug('🌍 PostHog: Geolocation failed, assuming non-EEA user:', geoError);
      }
      return false;
    }
  } catch (error) {
    if (process.env.NODE_ENV === 'development') {
      console.debug('🌍 PostHog: Location detection failed, assuming non-EEA user:', error);
    }
    return false;
  }
}
...
if (typeof window !== 'undefined') {
  ...
  isEEAUser().then((userIsInEEA) => {
    ...
    if (userIsInEEA) {
      ...
      userTypeReason = shouldInitialize ? 'EEA user with consent' : 'EEA user without consent';
    ...
    if (shouldInitialize && process.env.NEXT_PUBLIC_POSTHOG_KEY) {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.warn('⚠️ PostHog not initialized - missing NEXT_PUBLIC_POSTHOG_KEY');
      }
    }
  });
  ...
  window.addEventListener('storage', (event) => {
    if (event.key === 'c15t-consent-state' && !posthog.__loaded) {
      if (process.env.NODE_ENV === 'development') {
        console.log('🍪 Consent change detected, checking if PostHog should initialize...');
      }
      ...
      isEEAUser().then((userIsInEEA) => {
        if (userIsInEEA) {
          ...
          if (newConsentGiven && process.env.NEXT_PUBLIC_POSTHOG_KEY) {
            ...
            if (process.env.NODE_ENV === 'development') {
              console.log('🔍 PostHog initialized after EEA user granted consent');
              addPostHogDevIndicator(userTypeReason);
            }
          }
        }
