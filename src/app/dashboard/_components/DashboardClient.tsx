...
const ListeningTrends = lazy(() => import('@/features/visualization/ListeningTrends'));
...
interface ApiRestrictionNoticeProps {
  readonly onDismiss: () => void;
}

function ApiRestrictionNotice({ onDismiss }: Readonly<ApiRestrictionNoticeProps>) {
  return (
        ...
        <div className="flex items-start">
          ...
          <div>
            ...
            <p className="text-xs text-yellow-200 mb-3">
              Due to Spotify&apos;s new API restrictions requiring 250k MAUs (Monthly Active Users)
              and only accepting organizations (not individuals), this app has severe rate limits
              and you may experience slower performance.
            </p>
...
export default function DashboardClient({
  ...
  return (
    <>
      ...
      {showApiNotice && <ApiRestrictionNotice onDismiss={() => setShowApiNotice(false)} />}
      ...
      {/* Visualizations */}
      <section aria-label="Music Analytics Visualizations" className="space-y-8 mb-10">
        <h2 className="text-2xl font-bold text-spotify-white mb-4">Your Music Journey</h2>
          ...
          <div className="flex-1 space-y-6 mt-6">
            {activeTab === 'artists' && (
              <article aria-label="Top artists analysis">
                <h3 className="text-xl font-semibold text-spotify-white mb-4">Your Top Artists</h3>
                <TopArtists
                  artists={topArtists}
                  isLoading={false}
                  error={error}
            ...
            {activeTab === 'tracks' && (
              <article aria-label="Top tracks analysis">
                <h3 className="text-xl font-semibold text-spotify-white mb-4">Your Top Tracks</h3>
                <TopTracks tracks={topTracks} isLoading={false} error={error} onRetry={refresh} />
              </article>
            )}

            {activeTab === 'genres' && (
              <article aria-label="Top genres analysis">
                <h3 className="text-xl font-semibold text-spotify-white mb-4">Your Top Genres</h3>
                <TopGenres artists={topArtists} isLoading={false} error={error} onRetry={refresh} />
              </article>
            )}

            {activeTab === 'recent' && (
              <article aria-label="Recently played music">
                <h3 className="text-xl font-semibold text-spotify-white mb-4">Recently Played</h3>
                <RecentlyPlayed
