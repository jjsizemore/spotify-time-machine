...
export function middleware(request: NextRequest) {
  ...
  const stateCookie = request.cookies.get('next-auth.state') ||
  ...
  if (request.nextUrl.pathname.startsWith('/api/auth/callback')) {
    console.log('[MIDDLEWARE] /api/auth/callback hit. State cookie value:', stateCookie?.value);
    // If the state cookie is missing during callback, redirect to sign in
    if (!stateCookie && request.nextUrl.searchParams.has('state')) {
      console.error('[MIDDLEWARE] State cookie MISSING on callback with state param. Redirecting to /.');
      return NextResponse.redirect(new URL('/', request.url));
    }
    console.log('[MIDDLEWARE] State cookie present or no state param on callback. Proceeding.');
